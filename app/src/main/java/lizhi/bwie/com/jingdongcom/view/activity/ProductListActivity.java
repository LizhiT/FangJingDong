...
import lizhi.bwie.com.jingdongcom.presenter.ProductListPresenter;
import lizhi.bwie.com.jingdongcom.util.ApiUtil;
import lizhi.bwie.com.jingdongcom.view.adapter.ProDuctGridAdapter;
import lizhi.bwie.com.jingdongcom.view.adapter.ProDuctListAdapter;
...
//        条目的点击事件 调到详情页面
                ...
                proDuctListAdapter.setOnItemListner(new OnItemListner() {
                    @Override
                    public void onItemClick(int position) {
                        Toast.makeText(ProductListActivity.this, productListBean.getCode(), Toast.LENGTH_SHORT).show();
                        //跳转详情
                        Intent intent = new Intent(ProductListActivity.this, DetailActivity.class);
                        intent.putExtra("pid", listAll.get(position).getPid());
                        startActivity(intent);
